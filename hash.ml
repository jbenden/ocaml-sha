...
module Sha1dc = Make (
  struct
    ...
    external to_bin: t -> string = "%identity"
    external init: unit -> ctx = "stub_sha1dc_init"
    external unsafe_update_substring: ctx -> string -> int -> int -> unit = "stub_sha1dc_update"
    external update_buffer: ctx -> buf -> unit = "stub_sha1dc_update_bigarray"
    external finalize: ctx -> t = "stub_sha1dc_finalize"
  end
  )
